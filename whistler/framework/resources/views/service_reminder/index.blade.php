    ...
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">
          ...
          &nbsp;
          @can('ServiceReminders add')<a href="{{ route('service-reminder.create')}}" class="btn btn-success"
        ...
        <table class="table" id="data_table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                @if($reminder->preventive_maintenance->vehicle['vehicle_image'] != null)
                <img src="{{asset('uploads/'.$reminder->preventive_maintenance->vehicle['vehicle_image'])}}" height="70px" width="70px">
                @else
                <img src="{{ asset(" assets/images/vehicle.jpeg")}}" height="70px" width="70px">
                @endif
              </td>
              <td>
                {{$reminder->preventive_maintenance->vehicle->vehicleData->make}} {{$reminder->preventive_maintenance->vehicle->vehicleData->model}} - {{
                $reminder->preventive_maintenance->vehicle->license_plate }}
              </td>
              <td>
                {{$reminder->preventive_maintenance->services['description']}}
              </td>
              <td>
                {{date($date_format_setting,strtotime($reminder->last_date))}}
              </td>
              <td>
                {{$reminder->last_meter}} {{Hyvikk::get('dis_format')}}
              </td>
              <td>
                {{$reminder->planned_at}} {{Hyvikk::get('dis_format')}}
              </td>
              <td>
                {{ implode(', ', explode(',', $reminder->preventive_maintenance->email_to)) }}
              </td>
              ...
              {{-- <td>
                ...
                @php ($diff_in_days = $to->diffInDays($from, false))
                @if ($diff_in_days >= 0)
                @lang('fleet.after') {{$diff_in_days}} @lang('fleet.days')
                @else
                @lang('Overdue By') {{ abs($diff_in_days) }} @lang('fleet.days')
                @endif
              </td>
              ...
              {{-- <td>
                ...
                @can('ServiceReminders delete')<button type="button" class="btn btn-danger" data-id="{{$reminder->id}}"
