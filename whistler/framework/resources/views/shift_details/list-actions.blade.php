<td>
    <a href="{{ route('shift-details.edit', $row->id) }}" class="btn btn-sm btn-secondary m-1">Edit</a>
    @if (Auth::user()->user_type == 'O' || Auth::user()->user_type == 'EO' && Auth::user()->group_id == 1 ||
    Auth::user()->group_id == 2 )
    <form action="{{ route('shift-details.destroy', $row->id) }}" method="POST" style="display: inline-block;">
        @csrf
        @method('DELETE')
    ...
    @if ($row->status)
    <form action="{{ route('shift-details.approve', $row->id) }}" method="POST" style="display: inline-block;">
        @csrf
        @method('PUT')
