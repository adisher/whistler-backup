      ...
      <div class="card-header">
        <h3 class="card-title">
          ...
          @can('Fuel add')
            Allocate Fuel 
            &nbsp;
            <a href="{{ route('fuel.create')}}" class="btn btn-success"  title="@lang('fleet.addNew')">
          ...
          <table class="table allocation-table" id="data_table">
              ...
              <tr>
                ...
                <td>
                  <a href="{{ url("admin/vehicles/".$row->vehicle_id."/edit")}}">
                    ...
                    {{$row->vehicle_data->vehicleData->make}} - {{$row->vehicle_data->vehicleData->model}}
                  </a>
                  <br>
                  <b>Fleet No:</b> {{$row->vehicle_data->fleet_no}}
                  <br>
                  <b>Type:</b> {{$row->vehicle_data->types->displayname}}
                </td>
                ...
                {{-- <td>
                  ...
                  {{ $currency }} {{$total}}
                  <br>
                  {{ $currency }} {{$row->cost_per_unit}}/Liter
                </td> --}}
                @if($row->vehicle_data->type_id == 1)
                <td>
                  <b>Quantity:</b> {{$row->qty}} Liters
                  <br>
                  <b>Meter:</b> {{$row->meter}} {{Hyvikk::get('dis_format')}}
                  <br>
                </td>
                @elseif($row->vehicle_data->type_id == 2)
                <td>
                  <b>Quantity:</b> {{$row->qty}} Liters
                  <br>
                  <b>Hours:</b> {{$row->time}}
                ...
                <td>
                  <div class="btn-group">
                    ...
                    <div class="dropdown-menu custom" role="menu">
                      @can('Fuel edit')
                      <form action="{{ url("admin/fuel/".$row->id."/edit") }}" method="GET">
                        @csrf
                        <input type="hidden" name="fleet_id" id="get_fleet_id" value="{{ $row->vehicle_id }}">
            ...
            <tfoot>
              <tr>
                <th>
                  @if($allocation->count() > 0)
                  @can('Fuel delete')<button class="btn btn-danger" id="bulk_delete" data-toggle="modal" data-target="#bulkModal"
                    disabled title="@lang('fleet.delete')"><i class="fa fa-trash"></i></button>@endcan
                  @endif
                </th>
                {{-- <th>#</th> --}}
                <th>Image</th>
